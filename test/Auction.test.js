...
contract('Auction', ([owner, user1, user2]) => {
	...
	beforeEach(async () => {
		...
		// Initialize auction
		auction = await Auction.new(user1, user1, 0, web3.utils.toWei('1', 'ether'), 3600, nft.address, twitter.address);
		// Approve auction contract to transfer nft ownership
		await nft.approve(auction.address, 0, { from: user1 });
	});

	describe('Bidding', () => {
		describe('Success', () => {
			it('updates the highest bidder after each bid', async () => {
				await auction.bid({ from: user2, value: web3.utils.toWei('1', 'ether') });
				const highestBidder = await auction.getHighestBidder();
				highestBidder.should.equal(user2);
			});
			...
			it('emits a "HighestBidIncreased Event"', async () => {
				const bid = await auction.bid({ from: user2, value: web3.utils.toWei('1', 'ether') });
		...
		describe('Failure', () => {
			...
			it('does not allow a bid under the starting price', async () => {
				await auction
					.bid({ from: owner, value: web3.utils.toWei('0.5', 'ether') })
					.should.be.rejectedWith(helpers.EVM_REVERT);
			});

			it('does not allow consecutive bids from same user', async () => {
				await auction.bid({ from: owner, value: web3.utils.toWei('1', 'ether') });
				await auction
			...
			it('does now allow bids after the auction end time', async () => {
				...
				await auction.endAuction({ from: user1 });
				await auction
					.bid({ from: owner, value: web3.utils.toWei('2', 'ether') })
					.should.be.rejectedWith(helpers.EVM_REVERT);
			});
		});
	});
	...
	describe('Ending Auction', () => {
		describe('Success', () => {
			it('updates balances of seller & twitter contract successfully', async () => {
				...
				let initialOriginalOwnerBalance = web3.utils.toBN(await web3.eth.getBalance(user1));
				let initialTwitterContractBalance = web3.utils.toBN(await web3.eth.getBalance(twitter.address));

				// Bid
				await auction.bid({ from: user2, value: web3.utils.toWei('2', 'ether') });

				// End the auction
				await time.increase(time.duration.hours(1));
				const event = await auction.endAuction({ from: user1 });
				...
				// Get final balances
				let finalOriginalOwnerBalance = web3.utils.toBN(await web3.eth.getBalance(user1));
			...
			it('emits an AuctionEnded event', async () => {
				...
				data.winner.should.equal(user2);
