...
contract TweetNFT is ERC721, ERC721URIStorage {
    ...
    function createAuction(uint256 _nftId, uint256 _startingPrice, uint256 _auctionDuration) external {
        require(isApprovedOrOwner(msg.sender, _nftId), "Caller is not owner nor approved");
        Auction newAuction = new Auction(payable(_originalOwners[_nftId]), payable(msg.sender), _nftId, _startingPrice, _auctionDuration, address(this));
